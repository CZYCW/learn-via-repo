---
name: learn-via-repo
description: "Systematically learn any software engineering domain by deeply reading one open-source repo. Works for AI Agents, Web frameworks, databases, OS kernels, compilers, game engines, distributed systems, and more. Handles three entry modes: (A) user brings a specific repo to learn, (B) user knows the domain but needs a repo recommended, (C) user is exploring and unsure. Hands-on first: assesses student level (5 grades), recommends an authoritative beginner repo built from first principles (no framework abstractions like langchain/spring/sqlalchemy) for stage-0 prerequisites if needed. Auto-researches external materials + canonical minimal implementations, explores the target repo, generates a complete Diátaxis-organized teaching system (stage-0 fundamentals → stage-1 foundations → stage-2 architecture → stage-3 walkthroughs → stage-4 applied + cross-cutting reference / exercises / resources / meta). Teaches via 最简对照法 (referencing real running code, not AI-improvised) + Feynman checks + semantic waves + 3-layer progression, with hard checkpoints A/B. USE when: user says 'teach me X through repo Y' / '我想系统学习 X' / 'help me learn X by reading a repo' / invokes /learn-via-repo. DO NOT USE for: quick code questions, code review, or building things without learning intent."
---
